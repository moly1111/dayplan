...
- 支持双击空白区域直接创建任务
- 任务完成后自动折叠并显示 🎉 庆祝图标
- Focus 任务完成后在 Completed 列表显示 ⭐ 星标
- 可点击标题手动展开/折叠
- 折叠状态下不可拖入新任务
...
- 已完成区块展开时（含空列表）有最小高度与内边距，便于区分展开/闭合
- 长按任务 300ms 触发拖拽模式，鼠标变为抓取样式
- Focus 已有任务或处于折叠状态时，无法拖入新任务
- Focus 任务具有与普通任务相同的全部功能（预估时间、计时、编辑、删除等）
- 每个日期的 Focus 折叠状态和完成状态独立保存
- 左右拖拽移动任务时，任务会保留所有信息（预估时间等），但变为普通任务
- 双击日历区域（包括空白）可快速跳回今天
